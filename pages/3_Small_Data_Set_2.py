...
The sites in the Central Time Zone are selected, the aggregated net load of which is considered as the target.
The data from 01/01/2015 to 10/31/2015 are used for training, and the remaining data are used for testing.

First, we select the sites with time zone `America/Chicago` (the Central Prevailing Time).
'''
...
- The original timestamps are in the Central Prevailing Time, which is converted to UTC.
'''

st.code('''
path = 'raw/SunDance_data_release/energy/'
df = pd.DataFrame({'utc': [], 'cpt': [], 'kw': []})
for file in files:
    if isfile(path + file):
    ...
    if (len(df1) >= 8760) and ('Grid [kW]' in df1.columns):
        df1['cpt'] = df1['Date & Time']
        df1 = df1[df1['cpt'].dt.year == 2015][::-1]
        df1['cpt'] = df1['cpt'].dt.tz_localize('US/Mountain', ambiguous='infer')
        df1['utc'] = df1['cpt'].dt.tz_convert(None)
        df1['cpt'] = df1['cpt'].dt.tz_localize(None)
        df1['kw'] = df1['Grid [kW]']
        df = pd.concat([df, df1[['utc', 'cpt', 'kw']]])
df = df.groupby(['utc', 'cpt'], as_index=False).sum()
''')
...
st.code('''
weather = pd.read_csv('raw/AUS.csv', parse_dates=['valid']).drop(columns='station')
...
'''
As an example, we augment the feature space.
...
with st.echo():
    df = pd.read_csv('data/S2.csv', parse_dates=['utc', 'cpt'])
    df['month'] = df['cpt'].dt.month
    df['hour'] = df['cpt'].dt.hour
    df['dayofweek'] = df['cpt'].dt.dayofweek
    df['kw_2'] = df['kw'].shift(24 * 2)
    df['kw_7'] = df['kw'].shift(24 * 7)

'''
We display the resulting dataframe as an interactive table.
'''
